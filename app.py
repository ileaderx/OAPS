...
def before_request():
    ...
    ip = ipService.find_ip_by_ip(addr, IP)
    if ip is None:
...
def subject(subject_id):
    subject = subjectService.find_by_id(subject_id, Subject)
    articles = articleService.find_by_subject(subject.id, Article)
    articles.reverse()
    particles = []
...
def article(article_id):
    ...
    user = userService.find_by_id(article.user_id, User)
    ip = ipService.find_ip_by_ip(request.remote_addr, IP)
    aip = ipService.find_aip_both(article_id,ip.id,ArticleIp)
...
def article_upvote(article_id):
    ip = ipService.find_ip_by_ip(request.remote_addr,IP)
    articleService.upvote(article_id,ip.id,ArticleIp,Article)
...
def article_downvote(article_id):
    ip = ipService.find_ip_by_ip(request.remote_addr,IP)
    articleService.downvote(article_id, ip.id,ArticleIp,Article)
    return 'vote successfully!'
'''
    文章评论
'''
...
def article_comment(article_id):
    ...
    ip = ipService.find_ip_by_ip(request.remote_addr,IP)
    cip = ipService.find_cip_by_both(comment.id,ip.id,CommentIp)
    if cip is None:
...
def cupvote(comment_id):
    ip = ipService.find_ip_by_ip(request.remote_addr,IP)
    commentService.upvote(comment_id, ip.id,CommentIp,Comment)
    return 'vote successfully!'

@app.route('/cdownvote/<comment_id>')
def cdownvote(comment_id):
    ip = ipService.find_ip_by_ip(request.remote_addr,IP)
    commentService.downvote(comment_id, ip.id,CommentIp,Comment)
    return 'vote successfully!'
...
def psw_manage():
    form = request.form
    psw = passwordService.get_password(Password)
    oldpsw = form['old']
