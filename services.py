...
class SubjectService:
    '''
        ...
        3.通过id获取主题
    '''
    def find_all_subject(self, Subject):
        subjects = Subject.query.all()
        return subjects

    def find_by_title(self,title, Subject):
        subjects = Subject.query.filter_by(title=title).first()
        return subjects

    def find_by_id(self,id,Subject):
...
class ArticleService:
    ...
    def find_by_id(self,id, Article):
        article = Article.query.filter_by(id=id).first()
        return article

    def find_by_subject(self,subject_id, Article):
    ...
    #返回当前最大id
    def nextId(self, Article):
        return db.session.query(db.func.max(Article.id)).one()[0]+1

    #点赞,0-默认，1-点赞，2-差评
    def upvote(self,article_id,ip_id,ArticleIp, Article):
        aip = ipService.find_aip_both(article_id,ip_id,ArticleIp)
    ...
    def downvote(self,article_id,ip_id,ArticleIp, Article):
        aip = ipService.find_aip_both(article_id, ip_id,ArticleIp)
        article = articleService.find_by_id(article_id,Article)
        if aip.vote_state == 2:
            aip.vote_state = 0
    ...
    def search(self, content,Article):
        articles=[]
...
class IPService:
    def find_ip_by_ip(self,addr, IP):
    ...
    def find_aip_both(self,article_id,ip_id, ArticleIp):
    ...
    def find_cip_by_both(self,comment_id,ip_id,CommentIp):
...
class CommentService:
    ...
    def find_by_articleid(self,article_id,Comment):
    ...
    #点赞,0-默认，1-点赞，2-差评
    def upvote(self,comment_id,ip_id,CommentIp,Comment):
        cip = ipService.find_cip_by_both(comment_id,ip_id,CommentIp)
        comment = commentService.find_by_id(comment_id,Comment)
        if cip.vote_state == 1:
            cip.vote_state = 0
            comment.upvoteNum -= 1
    ...
    def downvote(self,comment_id,ip_id,CommentIp,Comment):
        cip = ipService.find_cip_by_both(comment_id, ip_id,CommentIp)
        comment = commentService.find_by_id(comment_id,Comment)
        if cip.vote_state == 2:
            cip.vote_state = 0
    ...
    def search(self, content, Comment):
